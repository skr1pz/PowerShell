	...
	If ($employee_Status -eq 'A') {
        ...
        Try {
            ...
            If (Get-ADUser -Filter {(enabled -eq "True") -and (employeeID -eq $employee_ID)}) {
                ...
                -Company        $company `
                -Manager        $manager_ID `
                `  <# altSecurityIdentities is where the Domino distinguishedName gets stored #> `
                -Add @{'altSecurityIdentities'=$dn} }
            ...
            Catch {
                  ...
                  $output + " value not found in Workday or multiple enabled users in AD with the same employeeID: " + $error[0].ToString() | Out-File $logfile -Append
...
If ($errorLog) {
                ...
                Send-MailMessage -SmtpServer mail.int.uhs.com -To mdmayer@uhs.com -From mdmayer@uhs.com -Subject "AD Update Script Error Log" -Body "ErrorLog file from updateADUserFromWorkday" -attachments $errorLog
...
Send-MailMessage -SmtpServer mail.int.uhs.com -To mdmayer@uhs.com -From mdmayer@uhs.com -Subject "AD Update Script Transcript" -Body "Transaction log file from updateADUserFromWorkday" -attachments $transcript